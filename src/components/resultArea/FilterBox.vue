  ...
  <div class="filterBox bg-white border p-4 mb-2">
    ...
    <div class="grid grid-cols-[300px_1fr_300px] mobile:grid-cols-1">
      <div
        ...
        :class="dataStore.command === 'reactions' ? '' : 'hidden'"
      >
        <p>篩選心情：</p>
        <select
          @change="filterAll"
          class="select select-bordered select-sm w-8/12"
          v-model="filterState.reaction"
        >
      ...
      <div :class="dataStore.command === 'reactions' ? 'hidden' : ''">
        <div class="form-control">
          <label class="label justify-start cursor-pointer">
            ...
            <input
              @change="filterAll"
              type="checkbox"
              v-model="filterState.removeDuplicate"
        ...
        <div
          ...
          :class="dataStore.command === 'comments' ? '' : 'hidden'"
        >
          <label class="label">
      ...
      <div>
        <div
          class="w-full flex justify-start items-center mt-10 mobile:my-4"
          :class="dataStore.command === 'comments' ? '' : 'hidden'"
        >
          ...
          <datepicker
            ...
            class="pl-2 border inline-block mobile:w-32 shrink"
      ...
      <div class="text-center">
        <div class="form-control">
          <label class="input-group input-group-sm">
            ...
            <input
              @input="searchKeyWord"
              v-model="filterState.searchName"
              type="text"
              class="rounded-none w-full pl-2 border input-bordered max-w-xs"
              placeholder="搜尋名字"
            />
          </label>
        </div>
        <div
          ...
          :class="dataStore.command.includes('comments') ? '' : 'hidden'"
        >
          <label class="input-group input-group-sm">
            <span class="whitespace-nowrap">搜尋{{ commandWord }}</span>
            <input
              @input="searchKeyWord"
              v-model="filterState.searchComment"
              type="text"
              class="rounded-none w-full pl-2 border input-bordered max-w-xs"
              :placeholder="'搜尋' + commandWord"
            />
          </label>
        </div>
      </div>
...
const commandWord = computed(() => {
  return dataStore.command.includes("comments") ? "留言" : "分享";
});
...
const onChangeTime = (time) => {
  const t = dayjs(time);
  const d = dayjs(filterState.filterTime)
    .set("hour", t.hour())
    .set("minute", t.minute());
...
const filterAll = () => {
  ...
  //搜尋留言
  if (
    filterState.searchComment !== "" &&
    dataStore.command.includes("comments")
  ) {
    rawData = rawData.filter((item) => {
